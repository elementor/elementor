...
use Elementor\Tests\Phpunit\Elementor\App\ImportExportCustomization\Test_Module;
...
class Test_Import extends Elementor_Test_Base {
	...
	public function test_remap_component_instances__maps_source_component_post_ids() {
		// Arrange
		$import = new Import( static::MOCK_KIT_ZIP_PATH, [ 'include' => [] ] );
		$method = new \ReflectionMethod( Import::class, 'remap_component_instances' );
		$method->setAccessible( true );

		$source_component_id = 93;
		$destination_component_id = 167;
		$unmapped_id = 104;
		...
		// Act
		$result = $method->invoke( $import, $elements, $post_ids_map );
	...
	public function test_run__import_site_settings() {
		...
		$site_settings = ImportExportCustomizationUtils::read_json_file( $extracted_directory_path . 'site-settings' );
	...
	private function assert_valid_elementor_content( $result, $manifest, $zip_path) {
		...
		foreach ( $manifest['content'] as $elementor_post_type => $elementor_posts ) {
			foreach ( $elementor_posts as $post_id => $post_settings ) {
				$expected_post_data = ImportExportCustomizationUtils::read_json_file( $import_process_tmp_dir . '/content/' . $elementor_post_type . '/' . $post_id );
