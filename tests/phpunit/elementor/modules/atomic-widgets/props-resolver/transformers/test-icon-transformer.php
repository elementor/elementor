...
class Test_Icon_Transformer extends Elementor_Test_Base {
	...
	public function test_transform__renders_svg_from_fontello_config_when_library_has_no_sibling_files() {
		// Arrange.
		$additional_tabs = static function ( $tabs ) {
			$tabs['emo'] = [
				...
				'prefix' => 'emo-',
				'displayPrefix' => 'emo',
				'fetchJson' => site_url( '/emo.js' ),
				'url' => site_url( '/emo.css' ),
			];
		...
		add_filter(
			...
			static function ( $preempt, $args, $url ) {
				if ( false !== strpos( $url, 'config.json' ) ) {
					return [
						'headers' => [],
						'body' => wp_json_encode( [
							'units_per_em' => 1000,
		...
		// Assert.
		$this->assertStringContainsString( '<svg', $result['html'] );
		$this->assertStringContainsString( 'M10 10', $result['html'] );
		$this->assertNull( $result['url'] );
	}
