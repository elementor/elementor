...
class Test_Document_Migration_Integration extends Elementor_Test_Base {
	...
	public function test_email_action_uses_email_to_emails_migration_from_production_manifest() {
		// Arrange
		$post_id = 2001;

		$document_data = [
			[
				'id' => 'form-email-migration',
				'elType' => 'e-form',
				'settings' => [
					'email' => [
						'$$type' => 'email',
		...
		Migrations_Orchestrator::destroy();
		$orchestrator = Migrations_Orchestrator::make( ELEMENTOR_PATH . 'migrations' );
		...
		// Assert
		$email_setting = $document_data[0]['settings']['email'];
		$this->assertSame( 'emails', $email_setting['$$type'] );

		$this->assertSame( 'string-array', $email_setting['value']['to']['$$type'] );
		$this->assertCount( 1, $email_setting['value']['to']['value'] );
		$this->assertSame( 'admin@example.com', $email_setting['value']['to']['value'][0]['value'] );

		$this->assertSame( 'string-array', $email_setting['value']['cc']['$$type'] );
		$this->assertSame( 'cc@example.com', $email_setting['value']['cc']['value'][0]['value'] );

		$this->assertSame( 'string-array', $email_setting['value']['bcc']['$$type'] );
		$this->assertSame( 'bcc@example.com', $email_setting['value']['bcc']['value'][0]['value'] );
	}
}
