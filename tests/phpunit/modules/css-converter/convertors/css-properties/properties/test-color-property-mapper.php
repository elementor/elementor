...
/**
 * PHPUnit Tests for Color Property Mapper
 * 
 * Tests atomic widget compliance for Color_Prop_Type structure
 * Validates color property with various color formats
 * 
 * @group css-converter
 * @group color-mapper
 */
class Test_Color_Property_Mapper extends Elementor_Test_Base {
	...
	public function test_supports_color_property(): void {
		$this->assertTrue( $this->mapper->is_supported_property( 'color' ) );
		$this->assertFalse( $this->mapper->is_supported_property( 'background-color' ) );
	...
	public function test_hex_colors(): void {
		...
		foreach ( $hex_colors as $case_name => $case_data ) {
			$result = $this->mapper->map_to_v4_atomic( 'color', $case_data['input'] );
	...
	public function test_named_colors(): void {
		...
		foreach ( $named_colors as $named_color => $expected_hex ) {
			$result = $this->mapper->map_to_v4_atomic( 'color', $named_color );
	...
	public function test_css_keywords(): void {
		...
		foreach ( $keywords as $keyword ) {
			$result = $this->mapper->map_to_v4_atomic( 'color', $keyword );
	...
	public function test_invalid_colors(): void {
		...
		foreach ( $invalid_colors as $case_name => $color ) {
			$result = $this->mapper->map_to_v4_atomic( 'color', $color );
			$this->assertNull( 
				$result, 
	...
	public function test_non_string_value(): void {
		...
		foreach ( $invalid_types as $type_name => $value ) {
			$result = $this->mapper->map_to_v4_atomic( 'color', $value );
			$this->assertNull( 
				$result, 
