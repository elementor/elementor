...
class Document_Mutator_Test extends TestCase {
	...
	public function test_insert_subtree_preserves_existing_ids() {
		// Arrange
		$subtree = [
			'id' => 'my-custom-id',
			'elType' => 'container',
		...
		// Assert
		$inserted = $result[0];
		$this->assertSame( 'my-custom-id', $inserted['id'] );
		$this->assertSame( 'child-1', $inserted['elements'][0]['id'] );
		$this->assertNotEmpty( $inserted['elements'][1]['id'] );
		$this->assertNotSame( 'child-1', $inserted['elements'][1]['id'] );
	}
