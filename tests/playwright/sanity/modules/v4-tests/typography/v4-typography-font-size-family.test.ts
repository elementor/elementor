...
test.describe.skip( 'V4 Typography Combined Font Size and Family Tests @v4-tests', () => {
	...
	test( 'Heading and Paragraph font size and family apply correctly in editor and published page', async () => {
		// Add container for both widgets
		const containerId = await editor.addElement( { elType: 'container' }, 'document' );
		// Add widgets and open typography
		const headingId = await addWidgetAndOpenTypography( 'e-heading', containerId );
		...
		const paragraphId = await addWidgetAndOpenTypography( 'e-paragraph', containerId );
		...
		for ( const selector of [ SELECTORS.heading, SELECTORS.paragraph ] ) {
			const el = frame.locator( selector );
			await expect( el ).toBeVisible( { timeout: timeouts.expect } );
			await expect( el ).toHaveCSS( 'font-size', `${ FONT_SIZE }px`, { timeout: timeouts.expect } );
			const computedFamily = await el.evaluate( ( e ) => window.getComputedStyle( e ).fontFamily );
		...
		for ( const selector of [ SELECTORS.heading, SELECTORS.paragraph ] ) {
			...
			await expect( el ).toBeVisible( { timeout: timeouts.navigation } );
			await expect( el ).toHaveCSS( 'font-size', `${ FONT_SIZE }px`, { timeout: timeouts.expect } );
			const computedFamily = await el.evaluate( ( e ) => window.getComputedStyle( e ).fontFamily );
			expect( computedFamily.toLowerCase() ).toContain( FONT_FAMILY.name.toLowerCase() );
		}
