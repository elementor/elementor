...
import { deleteAllGlobalClasses, openClassManager, saveAndCloseClassManager } from '../global-classes/utils';
...
test.describe( 'Design System Sync - Typography V4→V3 @v4-tests', () => {
	...
	test( 'Synced V4 typography class appears in V3 widget with correct responsive font-size values', async () => {
		...
		await test.step( 'Add V4 widget and create class with responsive typography', async () => {
			...
			await editor.v4Panel.style.addGlobalClass( 'SyncTypoTest' );

			await editor.v4Panel.style.openSection( 'Typography' );
			await editor.v4Panel.style.setFontSize( 74, 'px' );

			await editor.changeResponsiveView( 'tablet' );
			await editor.v4Panel.style.setFontSize( 48, 'px' );
		...
		await test.step( 'Enable sync_to_v3 via Class Manager UI', async () => {
			...
			const classItem = page.locator( 'li[role="listitem"]' ).filter( { hasText: 'SyncTypoTest' } );
		...
		await test.step( 'Reload editor, add V3 heading widget, and apply synced global typography', async () => {
			...
			const syncedItem = page.locator( '.e-global__preview-item.e-global__typography' ).filter( { hasText: 'SyncTypoTest' } );
			await expect( syncedItem ).toBeVisible( { timeout: 10000 } );
			await syncedItem.click();
		} );

		await test.step( 'Verify desktop font-size is 74', async () => {
			...
			await expect( desktopFontSizeInput ).toHaveValue( '74' );
		} );

		await test.step( 'Switch to tablet and verify font-size is 48', async () => {
			await editor.changeResponsiveView( 'tablet' );

			const tabletFontSizeInput = page.locator( '.elementor-control-typography_font_size_tablet .elementor-slider-input input' );
			await expect( tabletFontSizeInput ).toHaveValue( '48' );
		} );
	} );
} );
