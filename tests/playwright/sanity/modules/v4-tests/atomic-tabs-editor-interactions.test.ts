...
test.describe( 'Atomic Tabs Editor Interactions @atomic-widgets', () => {
	...
	test( 'Undo and redo multiple tab deletions restores correct state', async () => {
		// Arrange - Add tabs element (starts with 3 tabs)
		const tabsId = await editor.addElement( { elType: tabsType }, 'document' );
		const tabsRoot = getTabsRoot( tabsId );
		await editor.selectElement( tabsId );
		await editor.waitForPanelToLoad();
		...
		// Verify initial state: 3 tabs
		await expect.poll( () => getIdsByType( tabsRoot, tabType ) ).toHaveLength( 3 );
		await expect.poll( () => getIdsByType( tabsRoot, tabContentType ) ).toHaveLength( 3 );

		// Act - Remove first tab (now 2 tabs)
		const listItems = menuItemsField.locator( 'ul.MuiList-root > li' );
		await listItems.first().hover();
		const removeButton1 = listItems.first().getByRole( 'button', { name: 'Remove' } );
		await expect( removeButton1 ).toBeVisible();
		await removeButton1.click();

		await expect.poll( () => getIdsByType( tabsRoot, tabType ) ).toHaveLength( 2 );
		await expect.poll( () => getIdsByType( tabsRoot, tabContentType ) ).toHaveLength( 2 );

		// Act - Remove another tab (now 1 tab)
		await listItems.first().hover();
		const removeButton2 = listItems.first().getByRole( 'button', { name: 'Remove' } );
		await expect( removeButton2 ).toBeVisible();
		await removeButton2.click();

		await expect.poll( () => getIdsByType( tabsRoot, tabType ) ).toHaveLength( 1 );
		await expect.poll( () => getIdsByType( tabsRoot, tabContentType ) ).toHaveLength( 1 );

		// Act - Undo until editor is empty (undo delete 2, undo delete 1, undo add tabs)
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		await editor.page.evaluate( () => ( window as any ).$e.run( 'document/history/undo' ) );
		await expect.poll( () => getIdsByType( tabsRoot, tabType ) ).toHaveLength( 2 );
		...
		// Editor should be empty (tabs element removed)
		await expect( tabsRoot ).not.toBeVisible();

		// Act - Redo until last action (redo add tabs, redo delete 1, redo delete 2)
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		await editor.page.evaluate( () => ( window as any ).$e.run( 'document/history/redo' ) );
		...
		// Assert - Should have exactly 1 tab after all redos
		await expect.poll( () => getIdsByType( tabsRoot, tabType ) ).toHaveLength( 1 );
		await expect.poll( () => getIdsByType( tabsRoot, tabContentType ) ).toHaveLength( 1 );

		// Verify the remaining tab is functional
		const remainingTab = getMenuTabs( tabsRoot ).first();
		const remainingContent = getTabContents( tabsRoot ).first();
		await expect( remainingTab ).toBeVisible();
		await expect( remainingContent ).toBeVisible();
		await expect( remainingTab ).toHaveAttribute( 'aria-selected', 'true' );
	} );
