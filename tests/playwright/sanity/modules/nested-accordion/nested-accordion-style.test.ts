...
test.describe( 'Nested Accordion Style Tests @nested-accordion', () => {
	...
	for ( const atomicExperimentState of atomicExperimentStateCases ) { // Todo: remove once experiment is merged
		...
		test( 'Content style tests: ' + testTitleSuffix, async ( { page }, testInfo ) => {
			const wpAdmin = new WpAdminPage( page, testInfo ),
		...
		test( 'Header style tests: ' + testTitleSuffix, async ( { page }, testInfo ) => {
			const wpAdmin = new WpAdminPage( page, testInfo ),
				editor = await wpAdmin.openNewPage(),
				container = await editor.addElement( { elType: 'container' }, 'document' );
		...
		test( 'Header style tests - Text Stroke and Shadow: ' + testTitleSuffix, async ( { page }, testInfo ) => {
			const wpAdmin = new WpAdminPage( page, testInfo ),
				editor = await wpAdmin.openNewPage(),
				container = await editor.addElement( { elType: 'container' }, 'document' );
