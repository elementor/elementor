...
test.describe( 'Nested Accordion experiment is active @nested-accordion', () => {
	...
	for ( const atomicExperimentState of atomicExperimentStateCases ) { // Todo: remove once experiment is merged
		...
		test( 'Nested Accordion Visual Regression Test: ' + testTitleSuffix, async ( { browser }, testInfo ) => {
			// Act
			const page = await browser.newPage(),
		...
		test( 'Nested Accordion stays full width in container Direction Row: ' + testTitleSuffix, async ( { page }, testInfo ) => {
			const wpAdmin = new WpAdminPage( page, testInfo ),
				editor = await wpAdmin.openNewPage(),
		...
		test( 'Nested Accordion with inner Nested Accordion: ' + testTitleSuffix, async ( { browser }, testInfo ) => {
			// Act
			const page = await browser.newPage(),
				wpAdmin = new WpAdminPage( page, testInfo ),
				editor = await wpAdmin.openNewPage(),
		...
		test( 'Accessibility inside the Editor: ' + testTitleSuffix, async ( { page }, testInfo ) => {
			// Arrange.
			const wpAdmin = new WpAdminPage( page, testInfo );
			await setup( wpAdmin );
			const editor = await wpAdmin.openNewPage(),
		...
		test( 'Accessibility on the Front End: ' + testTitleSuffix, async ( { page }, testInfo ) => {
			// Arrange.
			const wpAdmin = new WpAdminPage( page, testInfo );
			await setup( wpAdmin );
			const editor = await wpAdmin.openNewPage(),
