...
test.describe( 'Nested Tabs tests @nested-tabs', () => {
	...
	test( 'Accessibility inside the Editor', async ( { page }, testInfo ) => {
		...
		await test.step( 'Keyboard handling inside the Editor', async () => {
			...
			await expect.soft( tabTitleOne ).toBeFocused();
			await expect.soft( tabTitleOne ).toHaveAttribute( 'aria-selected', 'true' );
			await page.keyboard.press( 'Tab' );
			...
			await expect.soft( button1 ).toBeFocused();
			await page.keyboard.press( 'Tab' );
			await expect.soft( buttonBelow ).toBeFocused();
			await page.keyboard.press( 'Shift+Tab' );
			await expect.soft( button1 ).toBeFocused();
			...
			await expect.soft( tabTitleThree ).toBeFocused();
			await expect.soft( tabTitleThree ).toHaveAttribute( 'aria-selected', 'false' );
			await page.keyboard.press( 'ArrowLeft' );
			await expect.soft( tabTitleTwo ).toBeFocused();
			await expect.soft( tabTitleTwo ).toHaveAttribute( 'aria-selected', 'false' );
			await page.keyboard.press( 'Enter' );
			await expect.soft( tabTitleTwo ).toHaveAttribute( 'aria-selected', 'true' );
			await page.keyboard.press( 'Tab' );
			await expect.soft( button2 ).toBeFocused();
			await page.keyboard.press( 'Tab' );
			await expect.soft( buttonBelow ).toBeFocused();
		} );
	} );
	...
	test( 'Test the nested tabs behaviour when using container flex row', async ( { page }, testInfo ) => {
		...
		const tabButtonOne = await editor.getPreviewFrame().locator( '.e-n-tabs .e-n-tab-title >> nth=0' ),
			...
			tabButtonTwo = await editor.getPreviewFrame().locator( '.e-n-tabs .e-n-tab-title >> nth=1' ),
			...
			tabButtonThree = await editor.getPreviewFrame().locator( '.e-n-tabs .e-n-tab-title >> nth=2' ),
