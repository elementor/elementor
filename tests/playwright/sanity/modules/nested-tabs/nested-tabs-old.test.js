...
test.describe( 'Nested Tabs tests @nested-tabs', () => {
	...
	test( 'General test', async ( { page }, testInfo ) => {
		...
		await setup( wpAdmin, { 'nested-elements-html': 'inactive' } );
		...
		// Tests.
		await expect( await editor.getPreviewFrame().locator( `.elementor-element-${ widgetId }` ) ).toHaveClass( /elementor-widget-n-tabs-old/ );
	...
	test( 'Title alignment setting', async ( { page }, testInfo ) => {
		...
		await expect( editor.getPreviewFrame().locator( '.elementor-widget-n-tabs-old .e-n-tabs-heading .e-n-tab-title.e-active' ) ).toHaveCSS( 'justify-content', 'flex-start' );
	...
	test( `Check visibility of icon svg file when font icons experiment is active`, async ( { page }, testInfo ) => {
		const wpAdmin = new WpAdminPage( page, testInfo );
		await setup( wpAdmin, { e_font_icon_svg: 'active', 'nested-elements-html': 'inactive' } );

		const editor = new EditorPage( page, testInfo );
		await editor.gotoPostId( pageId );
		await editor.loadTemplate( templatePath );
		await editor.publishAndViewPage();
		await page.waitForSelector( '.elementor-widget-n-tabs-old' );

		// Set published page variables
		const icon = await page.locator( '.elementor-widget-n-tabs-old .e-n-tab-title .e-n-tab-icon svg:first-child' ).first(),
			activeTabIcon = await page.locator( '.elementor-widget-n-tabs-old .e-n-tab-title .e-n-tab-icon svg:last-child' ).first(),
			currentContext = page;
	...
	test( `Check the icon size on frontend`, async ( { page }, testInfo ) => {
		...
		// Set icon size.
		await editor.getPreviewFrame().locator( '.elementor-widget-n-tabs-old' ).hover();
		await editor.getPreviewFrame().locator( '.elementor-widget-n-tabs-old .elementor-editor-element-edit' ).first().click();
	...
	test( 'Check if the svg icons are visible on mobile display on the front end', async ( { page }, testInfo ) => {
		...
		await cleanup( wpAdmin, { e_font_icon_svg: 'inactive', 'nested-elements-html': 'inactive' } );
	} );
	...
	test( "Check widget content styling doesn't override the content container styling when they are used together", async ( { page }, testInfo ) => {
		...
		for ( const widgetIdentifier in widgetsToTest ) {
			const widgetToTest = widgetsToTest[ widgetIdentifier ],
				widgetSelector = `.elementor-widget-n-tabs-old.elementor-element-${ widgetToTest.widgetId }`,
