	...
	test('should convert basic text-decoration values correctly', async ({ page, request }) => {
		const htmlContent = `
			<div>
				...
				<span style="text-decoration: overline;">Overlined Span</span>
		...
		// Test overline - Note: span elements may be converted to other elements
		// depending on widget mapping configuration, so we'll test if any element has overline
		const elements = previewFrame.locator('*');
		const overlineFound = await elements.evaluateAll(els => 
			els.some(el => getComputedStyle(el).textDecorationLine === 'overline')
		);
		expect(overlineFound).toBeTruthy();
	...
	test('should handle text-decoration shorthand correctly', async ({ page, request }) => {
		const htmlContent = `
			<div>
				...
				<span style="text-decoration: overline dotted green;">Dotted Overline</span>
			</div>
		`;
		...
		// Test overline from shorthand - span may be converted to other element
		const elements = previewFrame.locator('*');
		const overlineFound = await elements.evaluateAll(els => 
			els.some(el => getComputedStyle(el).textDecorationLine === 'overline')
		);
	...
	test('should handle invalid and edge case values', async ({ page, request }) => {
		const htmlContent = `
			<div>
				...
				<span style="text-decoration: inherit;">Inherit Decoration</span>
			</div>
		`;
		...
		const elements = previewFrame.locator('*').filter({ hasText: 'Inherit Decoration' });
		if (await elements.count() > 0) {
			const spanDecoration = await elements.first().evaluate(el => getComputedStyle(el).textDecorationLine);
			// Inherit values should resolve to some valid value
			expect(spanDecoration).toBeDefined();
		}
	});
