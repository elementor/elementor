...
test.describe( 'Icon List', () => {
	test( 'Test vertical alignment of the icons', async ( { page }, testInfo ) => {
		...
		// Assert.
		await expect.soft( editor.getPreviewFrame().locator( '.elementor-icon-list-item' ).first() ).toHaveCSS( 'align-items', 'flex-start' );
		await expect.soft( editor.getPreviewFrame().locator( '.elementor-icon-list-icon' ).first() ).toHaveCSS( 'top', '10px' );
	} );
} );
