...
test.describe( 'Playing cards tests', () => {
	test.beforeAll( async ( { page }, testInfo ) => {
		...
		await editor.getPreviewFrame().locator( `.elementor-element-${ playingCardId } .e-playing-cards-item` );
		new Content( page, testInfo );
	} );
	test( 'Add playing cards to editor', async ( { page } ) => {
		await test.step( 'Fit Aspect hidden for Icons', async () => {
			await page.locator( '.elementor-repeater-add' ).click();
			await page.locator( '.elementor-repeater-add' ).click();

			const fitstCardItemcontrols = await page.locator( '.elementor-repeater-fields' ).first(),
				secondCardItemcontrols = await page.locator( '.elementor-repeater-fields' ).nth( 1 ),
				thirdCardItemcontrols = await page.locator( '.elementor-repeater-fields' ).nth( 2 ),
				forthCardItemcontrols = await page.locator( '.elementor-repeater-fields' ).nth( 3 );

			await fitstCardItemcontrols.click();
			await secondCardItemcontrols.click();
			await thirdCardItemcontrols.click();
			await forthCardItemcontrols.click();
		} );
