...
class Control_Deprecated_Notice extends Base_UI_Control {
	...
	public function content_template() {
		...
		if ( data.replacement ) {
			notice += '<br>' + sprintf( wp.i18n.__( 'It has been replaced by <strong>%1$s</strong>.', 'elementor' ), data.replacement );
		}
		if ( data.last ) {
			notice += '<br>' + sprintf( wp.i18n.__( 'Note that %1$s will be completely removed once %2$s %3$s is released.', 'elementor' ), data.widget, data.plugin, data.last );
		}
		#>
