...
class Compatibility {
	...
	public static function register_actions() {
		...
		if ( is_admin() || defined( 'WP_LOAD_IMPORTERS' ) ) {
			add_filter( 'wp_import_post_meta', array( __CLASS__, 'on_wp_import_post_meta' ) );
			add_filter( 'wxr_importer.pre_process.post_meta', array( __CLASS__, 'on_wxr_importer_pre_process_post_meta' ) );
		}

		add_action( 'elementor/maintenance_mode/mode_changed', array( __CLASS__, 'clear_3rd_party_cache' ) );

		// Enable floating buttons and link in bio experiment for all.
		// TODO Remove in version 3.26.
		add_filter( 'pre_option_elementor_experiment-floating-buttons', array( __CLASS__, 'return_active' ) );
		add_filter( 'pre_option_elementor_experiment-link-in-bio', array( __CLASS__, 'return_active' ) );
	}
	...
	public static function init() {
		...
		add_filter( 'wpseo_sitemaps_supported_post_types', array( __CLASS__, 'filter_library_post_type' ) );
		add_filter( 'wpseo_accessible_post_types', array( __CLASS__, 'filter_library_post_type' ) );
		...
		if ( function_exists( 'gutenberg_init' ) ) {
			add_action( 'admin_print_scripts-edit.php', array( __CLASS__, 'add_new_button_to_gutenberg' ), 11 );
		}
	}
	...
	private static function polylang_compatibility() {
		...
		add_filter( 'pll_copy_post_metas', array( __CLASS__, 'save_polylang_meta' ), 10, 4 );
	}
