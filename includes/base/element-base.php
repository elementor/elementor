...
abstract class Element_Base extends Controls_Stack {
	...
	public function on_import_replace_dynamic_content( $element, $map_old_new_post_ids ) {
		...
		if ( isset( $element['settings'][ $tags_manager::DYNAMIC_SETTING_KEY ] ) ) {
			foreach ( $element['settings'][ $tags_manager::DYNAMIC_SETTING_KEY ] as $dynamic_name => $dynamic_value ) {
				$tag_data = $tags_manager->tag_text_to_tag_data( $dynamic_value );
				$tag = $tags_manager->create_tag( $tag_data['id'], $tag_data['name'], $tag_data['settings'] );
				$tag->on_import_replace_dynamic_tag( $map_old_new_post_ids );
				$element['settings'][ $tags_manager::DYNAMIC_SETTING_KEY ][ $dynamic_name ] = $tags_manager->tag_data_to_tag_text( $tag->get_id(), $tag->get_name(), $tag->get_settings() );
			}
		}
