...
class Source_Local extends Source_Base {
    ...
    public function export_multiple_templates( array $template_ids ) {
        if ( ! is_array( $template_ids ) ) {
            return new \WP_Error( '404', 'No array passed to export_multiple_templates class method' );
        }
    ...
    public function import_template() {
        ...
        if ( ( ( $zip = new \ZipArchive() )->open( $import_file ) ) === true ) {
            ...
            foreach ( array_diff( scandir( $temp_path ), array( '.', '..' ) ) as $file ) {
                ...
                $data = json_decode( file_get_contents( $temp_path . '/' . $file ), true );
                ...
                $items[] = $this->get_item( $template_id );
            }
            ...
            return $items;
        } else if ( ! empty( ( $data = json_decode( file_get_contents( $import_file ), true ) ) ) ) {
