...
class BatchProcessor {
	 ...
	 */
	private function apply_batch_operations( array $operations ) {
		...
		foreach ( $operations as $index => $operation ) {
			try {
				$results[] = $this->apply_operation( $collection, $operation, $index );
	 ...
	 */
	private function apply_operation( Variables_Collection $collection, array $operation, int $index ): array {
		$type = $operation['type'];
		...
		return $this->$method( $collection, $operation, $index );
	}
...
// rigid system that is difficult to break
