 ...
 * Based on: Multiple atomic widgets use String_Prop_Type for position
 * Prop Type: /atomic-widgets/prop-types/string-prop-type.php
 * Expected: {"$$type": "string", "value": "relative"}
 * 
 * REQUIREMENTS:
 * - Must be valid CSS position value
 * - Used across multiple widget types
 * - String type with specific allowed values
 */
class Position_Property_Mapper extends Modern_Property_Mapper_Base {

	private const SUPPORTED_PROPERTIES = ['position'];
	private const ATOMIC_WIDGET = 'e-container'; // Primary widget, but used by multiple
	...
	public function map_to_v4_atomic( string $property, $value ): ?array {
		...
		$normalized_value = $this->normalize_position_value( $value );
	...
	private function normalize_position_value( $value ): ?string {
		if ( ! is_string( $value ) ) {
			return null;
		}
