...
class Dimension_Property_Mapper extends Property_Mapper_Base {
	...
	public function map_to_v4_atomic( string $property, $value ): ?array {
		...
		// Based on atomic widgets style-schema.php: all dimension properties use Size_Prop_Type
		$parsed = $this->parse_dimension_value( $this->normalize_dimension_value( $value ) );
		
		// Handle keyword values
		if ( in_array( $parsed, self::KEYWORD_VALUES, true ) ) {
			return $this->create_v4_property_with_type( $property, 'string', $parsed );
		}
		
		// Handle size values - convert to proper size structure
