...
class Box_Shadow_Property_Mapper implements Class_Property_Mapper_Interface {
	...
	public function supports( string $property, $value ): bool {
		return $property === 'box-shadow' && is_string($value) && strlen(trim($value)) > 0;
	}
	...
	private function parse_box_shadow( string $value ): array {
		...
		$shadow = [
			...
			'vOffset' => [ '$$type' => 'size', 'value' => [ 'size' => 0, 'unit' => 'px' ] ],
			'blur' => [ '$$type' => 'size', 'value' => [ 'size' => 0, 'unit' => 'px' ] ],
		...
		if (preg_match(self::COLOR_PATTERN, $val, $matches)) {
			...
			$shadow['color'] = [ '$$type' => 'color', 'value' => trim($color) ];
		}
		...
		if (count($sizes) > 0) $shadow['hOffset'] = $sizes[0];
		if (count($sizes) > 1) $shadow['vOffset'] = $sizes[1];
		if (count($sizes) > 2) $shadow['blur'] = $sizes[2];
		if (count($sizes) > 3) $shadow['spread'] = $sizes[3];
		
		if ($position === 'inset') {
			$shadow['position'] = [ '$$type' => 'string', 'value' => $position ];
		}
