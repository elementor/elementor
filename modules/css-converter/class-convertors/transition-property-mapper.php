...
class Transition_Property_Mapper implements Class_Property_Mapper_Interface {
	...
	public function map_to_schema( string $property, $value ): array {
		if ( $property === 'transition' ) {
			...
			if ( isset( $parsed['property'] ) ) {
				$result['transition-property'] = [ '$$type' => 'string', 'value' => $parsed['property'] ];
			}
			if ( isset( $parsed['duration'] ) ) {
				$result['transition-duration'] = [ '$$type' => 'string', 'value' => $parsed['duration'] ];
			}
			if ( isset( $parsed['timing-function'] ) ) {
				$result['transition-timing-function'] = [ '$$type' => 'string', 'value' => $parsed['timing-function'] ];
			}
			if ( isset( $parsed['delay'] ) ) {
				$result['transition-delay'] = [ '$$type' => 'string', 'value' => $parsed['delay'] ];
			}
