...
class Selector_Matcher_Engine {
	...
	public function find_matching_widgets( string $selector, array $widgets ): array {
		...
		// DEBUG: Log selector parsing for e-con selectors
		if ( strpos( $selector, 'e-con' ) !== false ) {
			$tracking_log = WP_CONTENT_DIR . '/css-property-tracking.log';
			file_put_contents( $tracking_log, date('[H:i:s] ') . "SELECTOR_MATCHER: Parsing '{$selector}' -> type: " . ($parsed_selector['type'] ?? 'unknown') . "\n", FILE_APPEND );
		}
		...
		foreach ( $widgets as $widget ) {
			...
			if ( $element_id && $this->widget_matches_parsed_selector( $widget, $parsed_selector, $widgets ) ) {
				...
				// DEBUG: Log matches for e-con selectors
				if ( strpos( $selector, 'e-con' ) !== false ) {
					$widget_classes = $widget['attributes']['class'] ?? '';
					file_put_contents( $tracking_log, date('[H:i:s] ') . "SELECTOR_MATCHER: '{$selector}' MATCHED widget {$element_id} with classes: {$widget_classes}\n", FILE_APPEND );
				}
			}
			
			if ( ! empty( $widget['children'] ) ) {
				$child_matches = $this->find_matching_widgets_recursively( $parsed_selector, $widget['children'] );
	...
	private function find_matching_widgets_recursively( array $parsed_selector, array $widgets ): array {
		$matching_element_ids = [];
		
		foreach ( $widgets as $widget ) {
			$element_id = $widget['element_id'] ?? null;
			
			if ( $element_id && $this->widget_matches_parsed_selector( $widget, $parsed_selector, null ) ) {
				$matching_element_ids[] = $element_id;
			}
	...
	private function match_complex_selector( array $widget, array $parsed_selector, ?array $all_widgets ): bool {
		...
		// DEBUG: Log complex selector matching attempts
		if ( strpos( $widget_classes, 'e-con' ) !== false ) {
			$tracking_log = WP_CONTENT_DIR . '/css-property-tracking.log';
			file_put_contents( $tracking_log, date('[H:i:s] ') . "COMPLEX_SELECTOR: Testing widget {$element_id} ({$widget_classes}) against target part\n", FILE_APPEND );
		}
		...
		if ( strpos( $widget_classes, 'e-con' ) !== false ) {
			file_put_contents( $tracking_log, date('[H:i:s] ') . "COMPLEX_SELECTOR: Chain validation for {$element_id}: " . ($chain_valid ? 'VALID' : 'INVALID') . "\n", FILE_APPEND );
		}
