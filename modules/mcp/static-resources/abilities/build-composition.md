...
- LINKS: a `link` prop is supported ONLY on these widgets: `e-button`, `e-div-block`, `e-flexbox`, `e-grid`, `e-heading`, `e-image`, `e-paragraph`, `e-svg`. On any other widget a `link` is skipped and reported in `warnings` (the composition still builds) — to link a non-linkable element, wrap it in a linkable container. Link PropValue shape: `{ "$$type": "link", "value": { "destination": { "$$type": "url", "value": "https://example.com" }, "isTargetBlank": { "$$type": "boolean", "value": true }, "tag": { "$$type": "string", "value": "a" } } }`
