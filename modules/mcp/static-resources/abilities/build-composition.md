...
- **Prop names must come from the widget schema (use elementor/get-widget-schema tool with the widget type). A key not in the schema, an unresolvable value, an unknown global class label, a bad interaction item, or unparseable CSS is skipped, the rest is saved, and each skip is a `fixable:` warning (also in `warning_details`). Correct only that field via `elementor/manage-elements`; do not rebuild the composition and do not treat the skip as unsupported.**
