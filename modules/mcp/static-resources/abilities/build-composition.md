...
When the user describes a design that repeats across posts/pages ("each project", "the product detail page", "make every blog post look like this", "each item links to a page like this"), that is ONE `single` / `single-<cpt>` template driven by dynamic data — NOT N duplicated pages. Never loop `manage-site-parts` `action: create` to make one page per item.
...
### V3 note (temporary — remove with ED-25071)
`theme-post-content` is a V3 widget that is not yet listed by `elementor/list-widget-schemas` or resolvable via `elementor/get-widget-schema` (returns `elementor_v3_not_supported`). The composition write path already accepts it — emit `<e-post-content>` in `xml_structure` with no `element_config` needed. Remove this note once ED-25071 whitelists theme-single widgets in schema discovery.
