...
Properties with `x-responsive: true` may also be overridden per breakpoint via the reserved `settings_variants` key: `[{ "breakpoint": "tablet", "props": { "<key>": <plain value> } }]`. Desktop values stay on the top-level keys. Send `settings_variants: []` to clear overrides. This is not the legacy v3 `_tablet` / `_mobile` suffix pattern.
