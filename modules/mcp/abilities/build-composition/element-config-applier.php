...
class Element_Config_Applier {
	 ...
	 *                                                                 instances) and must skip plain-value
	 *                                                                 resolution, which assumes plain scalars
	 *                                                                 and would corrupt already-built envelopes
	 *                                                                 such as an override's `override_key`.
	 *
	 * @return array{ error: ?\WP_Error, warnings: string[] }
	 */
	public function apply( array &$config_id_index, array $element_config, array $widget_configs, array $prebuilt_config_ids = [] ): array {
		$errors = [];
		$warnings = [];
		...
		foreach ( $element_config as $config_id => $settings ) {
			...
			$resolved = in_array( $config_id, $prebuilt_config_ids, true )
				? $settings
				: $this->resolve_settings_against_schema( $settings, $schema, $tag, $config_id, $errors, $warnings );
