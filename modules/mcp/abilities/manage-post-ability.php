...
class Manage_Post_Ability extends Abstract_Ability {
	...
	private function ability_description(): string {
		return __(
			'Create, update, trash, restore, delete, or write content for an Elementor v4 post in a single call. Operations: create | update | replace_content | append_content | update_element | add_classes | remove_classes | trash | restore | delete. The `elements` field accepts plain JSON nodes ({ widget, text, tag, url, image_id, image_url, alt, size, svg_id, svg_url, svg_markup, css, classes, children }); $$type wrappers are never required from agents. Supported widgets: container | div | flexbox | heading | paragraph | button | image | svg. SVG nodes accept svg_id / svg_url, or svg_markup (inline <svg>…</svg> markup, which is sanitized and uploaded to the media library on save). Build global classes via elementor/manage-global-classes and reference them by id in classes:[]. Breakpoints and pseudo-states are not part of this version. Root-level widgets without a container parent are auto-wrapped in an e-div-block; see the `normalized` field in the response for details. Element-scoped ops (update_element / add_classes / remove_classes) take element_id + small payload and patch one node in place; when the targeted post is the one open in the editor, the canvas updates surgically without a document reload.',
			'elementor'
		);
	}
