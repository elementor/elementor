...
class Update_Settings_Ability {
			...
			[
				...
				'input_schema' => [
					...
					'properties' => [
						...
						'settings' => [
							'type' => 'object',
							'description' => 'Partial document settings object; merged into existing settings.',
	...
	public function execute( $input ) {
		...
		if ( null === $settings ) {
			return new \WP_Error( 'invalid_settings', __( 'The settings object is required.', 'elementor' ), [ 'status' => 400 ] );
		}
		...
		if ( ! $document->is_editable_by_current_user() ) {
			return new \WP_Error( 'rest_cannot_edit', __( 'Sorry, you are not allowed to edit this document.', 'elementor' ), [ 'status' => 403 ] );
		}
		...
		if ( ! $saved ) {
			return new \WP_Error( 'save_failed', __( 'Could not save document settings.', 'elementor' ), [ 'status' => 500 ] );
		}
