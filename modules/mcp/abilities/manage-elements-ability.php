...
class Manage_Elements_Ability extends Abstract_Ability {
	...
	protected function get_definition(): Ability_Definition {
		return new Ability_Definition(
			...
			[
				'type' => 'object',
				'required' => [ 'status', 'results', 'post_id', 'preview_url', 'render_url', 'llm_instructions' ],
				'properties' => [
					'status' => [ 'type' => 'string' ],
					'results' => [ 'type' => 'array' ],
	...
	private function handle_bulk( Document $document, array $operations ): array {
		...
		foreach ( $operations as $index => $operation ) {
			...
			$tree = $outcome['tree'];
			$any_change = true;
			$last_success_element_id = $element_id;
		...
		if ( is_wp_error( $save_result ) || ! $save_result ) {
			...
			return $this->with_mutation_links( $response, $document, $last_success_element_id );
		}
	...
	private function with_mutation_links( array $response, Document $document, ?string $element_id = null ): array {
		return array_merge(
			$response,
