...
class Global_Class_Post {
	...
	/**
	 * Bulk-create posts using direct $wpdb multi-row INSERTs.
	 * Bypasses wp_insert_post hooks for performance.
	 *
	 * @param array $items Array of [ 'class_id' => string, 'label' => string, 'data' => array ].
	 * @return string[] Class IDs that were successfully created.
	 */
	public static function bulk_create( array $items ): array {
		if ( empty( $items ) ) {
			return [];
		}
		...
		foreach ( $items as $item ) {
			$post_placeholders[] = '(%s, %s, %s, %s, %s, %s, %s, %s, %s)';
			$post_values[] = $item['label'];
		...
		$wpdb->query( $wpdb->prepare(
			"INSERT INTO {$wpdb->posts} (post_title, post_type, post_status, post_date, post_date_gmt, post_modified, post_modified_gmt, post_content, post_excerpt) VALUES "
			. implode( ', ', $post_placeholders ),
			...$post_values
		) );
		...
		$inserted_ids = range( $first_id, $first_id + $count - 1 );

		$meta_values = [];
		$meta_placeholders = [];

		foreach ( array_values( $items ) as $i => $item ) {
			$post_id = $inserted_ids[ $i ];

			$meta_placeholders[] = '(%d, %s, %s)';
			$meta_values[] = $post_id;
			$meta_values[] = self::META_KEY_ID;
			$meta_values[] = $item['class_id'];

			$meta_placeholders[] = '(%d, %s, %s)';
			$meta_values[] = $post_id;
			$meta_values[] = self::META_KEY_DATA;
			$meta_values[] = maybe_serialize( $item['data'] );
		}
		...
		foreach ( $inserted_ids as $post_id ) {
			clean_post_cache( $post_id );
		}

		$created_ids = [];
		foreach ( array_values( $items ) as $item ) {
			$created_ids[] = $item['class_id'];
		}
