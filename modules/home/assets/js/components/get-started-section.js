...
const GetStarted = ( { ...props } ) => {
	...
	const mdGridColumns = hasSidebarPromotion ? 'repeat(3, 1fr)' : 'repeat(4, 1fr)';
	...
	return (
		<Paper elevation={ 0 } sx={ { py: 3, px: { xs: 3, md: 4 }, display: 'flex', flexDirection: 'column', gap: 2, borderRadius: 1, border: '1px solid rgba(0, 0, 0, 0.12)' } }>
			...
			<List sx={ { display: 'grid', gridTemplateColumns: { xl: 'repeat(4, 1fr)', md: mdGridColumns, xs: 'repeat(2, 1fr)' }, columnGap: { md: 9, xs: 7 }, rowGap: 3 } }>
				{
...
GetStarted.propTypes = {
	...
	homeScreenData: PropTypes.object.isRequired,
};
