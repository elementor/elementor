...
class Atomic_Svg extends Atomic_Widget_Base {
	...
	protected function render() {
		...
		if ( isset( $settings['link'] ) && ! empty( $settings['link']['href'] ) ) {
			$svg_html = sprintf( '<a class="%s" href="%s" class="e-link" target="%s"> %s </a>', $classes[self::LINK_BASE_STYLE_KEY], esc_url( $settings['link']['href'] ), esc_attr( $settings['link']['target'] ), $svg_html );
		}
