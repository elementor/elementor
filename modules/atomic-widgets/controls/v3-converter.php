...
class V3_Converter {
	...
	private function set_atomic_controls() {
		$filtered_controls = array_filter( $this->original_controls, fn ( $control ) => 'section' !== $control['type'] );
	...
	private function get_query_control( $control ) {
		...
		if ( $this->is_control_term_query( $control ) ) {
			$query_type = Term_Query::ENDPOINT;
			$included_types = null;
			$excluded_types = [];
		} else if ( $this->is_control_elementor_query( $control ) ) {
			$included_types = [ Source_Local::CPT ];
			$excluded_types = [];
		} else if ( $this->is_control_attachment_query( $control ) ) {
			$included_types = [ 'attachment' ];
			$excluded_types = [];
			$query_config[ Query_Base::IS_PUBLIC_KEY ] = false;
		} else if ( $this->is_control_user_query( $control ) ) {
