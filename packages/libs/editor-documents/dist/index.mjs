...
import { __privateOpenRoute as openRoute, __privateRunCommand as runCommand2 } from "@elementor/editor-v1-adapters";
function useActiveDocumentActions() {
  const document = useActiveDocument();
  const permalink = document?.links?.permalink ?? "";
  const save = useCallback(() => runCommand2("document/save/default"), []);
  const saveDraft = useCallback(() => runCommand2("document/save/draft"), []);
...
// src/hooks/use-open-document-in-new-tab.ts
import { useCallback as useCallback3 } from "react";
function useOpenDocumentInNewTab() {
  return useCallback3((id) => {
    const url = getUpdateUrl(id);
