...
describe('useElementChildren', () => {
	...
	it('should return empty object when container is not found', () => {
		...
		// Act.
		const { result } = renderHook(() => useElementChildren('element-1', { tabs: 'tab', accordions: 'accordion' }));
	...
	it('should re-compute when elementId dependency changes', () => {
		...
		// Act.
		const { result, rerender } = renderHook(({ elementId }) => useElementChildren(elementId, { tabs: 'tab' }), {
