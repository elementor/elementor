...
describe( 'useNestedElements', () => {
	...
	describe( 'createElements - resilient redo', () => {
		it( 'should gracefully handle redo when parent containers no longer exist', () => {
			// Arrange.
			const mockElement1 = createMockChild( 'element-1', 'button' );
			const mockElement2 = createMockChild( 'element-2', 'text' );
			...
			mockCreateElement
				.mockImplementationOnce( () => {
					throw new Error( 'Container with ID "parent-1" not found' );
				} )
			...
			// Assert.
			expect( mockCreateElement ).toHaveBeenCalledTimes( 4 ); // Initial 2 + redo 2 attempts

			// Verify both elements were attempted during redo.
			expect( mockCreateElement ).toHaveBeenNthCalledWith( 3, {
				containerId: 'parent-1',
				type: 'button',
				settings: { text: 'Click me' },
