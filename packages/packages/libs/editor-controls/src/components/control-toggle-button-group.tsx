...
const SplitButtonGroup = <TValue,>({ size = 'tiny', onChange, items, fullWidth, value }: SplitButtonGroup<TValue>) => {
...
const usePreviewButton = <TValue,>(items: ToggleButtonGroupItem<TValue>[], value: TValue) => {
	const [previewButton, setPreviewButton] = useState(items.find((item) => item.value === value) ?? items[0]);
