...
import { InfoAlert, MenuListItem } from '@elementor/editor-ui';
import { Select, type SelectChangeEvent, Typography } from '@elementor/ui';
import { AlertTitle, Box, Infotip } from '@elementor/ui';
...
export const SelectControl = createControl( ( { options, onChange, fallbackLabels = {} }: Props ) => {
	const { value, setValue, disabled, placeholder, tooltip } = useBoundProp( stringPropTypeUtil );
	...
	return (
		<ControlActions>
			<ConditionalControlInfotip isEnabled={ !! tooltip } { ...tooltip }>
				<Select
					sx={ { overflow: 'hidden' } }
					displayEmpty
					size="tiny"
