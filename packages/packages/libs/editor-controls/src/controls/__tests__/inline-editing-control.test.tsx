...
describe( 'InlineEditingControl Generate button', () => {
	...
	const getExpectedPrompt = () =>
		expect.stringMatching(
			new RegExp(
				`Generate or update the heading title for element ID ${ ELEMENT_ID }\\. Current title: "<p>Hello</p>"`
			)
		);

	it( 'renders the Generate button when enableAngieGenerate is true', () => {
		// Arrange
		renderControl( <InlineEditingControl enableAngieGenerate context={ { elementId: ELEMENT_ID } } />, {
			bind: 'title',
			setValue: jest.fn(),
			propType,
			value: { $$type: 'escaped-html', value: '<p>Hello</p>' },
	...
	it( 'does not render the Generate button when enableAngieGenerate is false', () => {
		// Arrange
		renderControl( <InlineEditingControl context={ { elementId: ELEMENT_ID } } />, {
			bind: 'title',
			setValue: jest.fn(),
			propType,
			value: { $$type: 'escaped-html', value: '<p>Hello</p>' },
