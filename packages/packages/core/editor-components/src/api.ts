...
export const apiClient = {
	...
	getComponentLockStatus: async (componentId: number) => 
		await httpService().get< { data: ComponentLockStatusResponse } >(`${BASE_URL_LOCK_STATUS}`, {
		 params: {
			componentId
		}
	}).then((res) => res.data),
	getComponentLockedUser: async (componentId: number) => {
		const response = await httpService().get<{   is_current_user_locked: boolean  }>(BASE_URL_IS_CURRENT_USER_LOCKED,{
		 params: {
			componentId
		}
		}).then((res) => res.data)
	...
	lockComponent: async (componentId: number) => 
		await httpService().post<{ success: boolean }>(LOCK_COMPONENT, {
			 componentId
		}).then(res=>res.data)
	,
	unlockComponent: async (componentId: number) => 
		await httpService().post<{ success: boolean }>(UNLOCK_COMPONENT, {
			 componentId
		}).then(res=>res.data)
	,	
};
...
export const canSwitchDocument = async (componentId: number): Promise<{
