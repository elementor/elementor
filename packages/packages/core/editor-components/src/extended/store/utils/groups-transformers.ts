...
export function movePropBetweenGroups(groups: Groups, propKey: string, fromGroupId: string, toGroupId: string): Groups {
...
export function removePropsFromState(
	...
	const updatedGroupItems = Object.fromEntries(
		Object.entries(overridableProps.groups.items).map(([groupId, group]: [string, OverridablePropsGroup]) => [
			groupId,
			{
				...group,
