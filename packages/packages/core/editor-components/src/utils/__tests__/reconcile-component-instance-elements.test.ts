import { getWidgetsCache, type V1ElementConfig, type V1ElementData } from '@elementor/editor-elements';
...
const PARENT_TYPE = 'e-test-parent';
const CHILD_TYPE = 'e-test-child';
const LEAF_TYPE = 'e-test-leaf';
...
describe( 'reconcileComponentInstanceElements', () => {
	...
	it( 'removes the child when the condition fails', () => {
		...
		const elements = [ createParent( { show_child: { $$type: 'boolean', value: false } }, [ existing ] ) ];
	...
	it( 'keeps an existing child untouched when the condition is met', () => {
		// Arrange.
		const existing = { id: 'child1', elType: CHILD_TYPE, elements: [] } as V1ElementData;
		const elements = [ createParent( { show_child: { $$type: 'boolean', value: true } }, [ existing ] ) ];
	...
	it( 'leaves elements without children_dependencies untouched', () => {
		// Arrange.
		const elements = [ { id: 'leaf1', elType: LEAF_TYPE, settings: {}, elements: [] } as V1ElementData ];
	...
	it( 'reconciles nested elements', () => {
		...
		// Assert.
		expect( result[ 0 ].elements?.[ 0 ].elements?.map( ( child ) => child.elType ) ).toEqual( [ CHILD_TYPE ] );
	} );
	...
	it( 'derives ids for the whole inserted subtree instead of random ones', () => {
		...
		expect( inserted?.id ).toBe( '1b3i53i' );
		expect( inserted?.elements?.map( ( leaf ) => leaf.id ) ).toEqual( [ '1twf30s', '0mjpz4d' ] );
	} );
	...
	it( 'derives distinct ids for different parents', () => {
		...
		const elements = [ createParent( settings, [], 'parent1' ), createParent( settings, [], 'parent2' ) ];
