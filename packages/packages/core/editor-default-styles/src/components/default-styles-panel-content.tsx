...
export function DefaultStylesPanelContent({ onRequestClose }: DefaultStylesPanelContentProps) {
	...
	const tagOptions = useMemo<Option[]>(() => allowedTags.map((tag) => ({ label: tag, value: tag })), [allowedTags]);
						...
						<Panel>
							<PanelHeader>
								<Stack
									...
									spacing={0.5}
								>
									<PanelHeaderTitle sx={{ flex: 1, minWidth: 0 }}>{__('Default Styles', 'elementor')}</PanelHeaderTitle>
									<CloseButton
