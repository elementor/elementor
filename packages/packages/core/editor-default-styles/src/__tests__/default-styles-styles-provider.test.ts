...
describe('defaultStylesStylesProvider', () => {
	...
	it('registers with priority between base and global', () => {
		const provider = stylesRepository.getProviders().find((entry) => entry.getKey() === DEFAULT_STYLES_PROVIDER_KEY);
	...
	it('returns class style definitions with prefixed cssName from all()', () => {
		...
		expect(defaultStylesStylesProvider.actions.resolveCssName(all[0].id)).toBe(`e-default-${all[0].id}`);
	});
});
