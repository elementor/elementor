...
jest.mock( '@elementor/editor-controls', () => ( {
	ControlActionsProvider: ( { children }: React.PropsWithChildren ) =>
		children,
	ControlReplacementsProvider: ( { children }: React.PropsWithChildren ) =>
		children,
...
jest.mock( '../allowed-tags', () => ( {
	...
	isAllowedDefaultStyleTag: ( tag: string, tags: string[] ) =>
		tags.includes( tag ),
} ) );
...
describe( 'DefaultStylesPanelContent', () => {
	let store: Store;
	...
	it( 'should render the stored active tag on the first paint', () => {
		...
		renderWithStore(
			<QueryClientProvider client={ queryClient }>
				<DefaultStylesPanelContent onRequestClose={ jest.fn() } />
			</QueryClientProvider>,
			store
		);
