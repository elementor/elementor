...
export const ZIndexField = ({ disabled }: { disabled?: boolean }) => {
	const StyleField = <StylesField bind="z-index" propDisplayName={ Z_INDEX_LABEL }>
	...
	const content = (
		...
		>
			...
			<Box component="span">
				{ __( 'z-index only works on positioned elements. Change position to relative, absolute, or fixed to enable layering.', 'elementor' ) }
			</Box>
		</Alert>
	);
	return disabled ? (
