...
export const CanvasInlineEditor = ( {
  ...
  const {
    ...
    clearAnchor,
  } = useRenderToolbar( rootElement.ownerDocument, id );
    ...
    <ThemeProvider>
      ...
      <InlineEditor
        ...
        editorProps={ {
          attributes: {
            style:
              'outline: none; display: inherit; justify-content: inherit; align-items: inherit; flex-direction: inherit; text-align: inherit;',
          },
        } }
      ...
      { toolbarAnchor && editor && (
        <InlineEditingToolbar anchor={ toolbarAnchor } editor={ editor } id={ id } />
      ) }
    </ThemeProvider>
  );
};
...
const InlineEditingOverlay = ( {
  ...
  const [ overlayRefElement, setOverlayElement ] = useState< HTMLDivElement | null >(
    inlineEditedElement
  );
  ...
  return overlayRefElement ? (
    <OutlineOverlay element={ overlayRefElement } id={ id } isSelected />
  ) : null;
};
