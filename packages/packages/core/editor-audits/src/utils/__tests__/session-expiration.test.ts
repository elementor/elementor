...
describe( 'refreshAuditsNonce', () => {
	...
	it( 'requests a fresh nonce from the WP core rest-nonce ajax action', async () => {
		// Arrange.
		const fetchMock = jest.fn().mockResolvedValue( { ok: true, text: () => Promise.resolve( 'fresh-nonce' ) } );
	...
	it( 'updates window.elementorAudits.nonce with the fresh nonce', async () => {
		...
		// Assert.
		expect( ( window as unknown as { elementorAudits: { nonce: string } } ).elementorAudits.nonce ).toBe(
			'fresh-nonce'
		);
	} );
	...
	it( 'throws a plain Error (not a SessionExpiredError) when the ajax request itself fails', async () => {
		// Arrange.
		global.fetch = jest.fn().mockResolvedValue( {
			ok: false,
			status: 500,
		...
		// Act & Assert.
		await expect( refreshAuditsNonce() ).rejects.not.toBeInstanceOf( SessionExpiredError );
		await expect( refreshAuditsNonce() ).rejects.toThrow( 'Failed to refresh audits nonce: HTTP 500' );
	} );

	it( 'deduplicates concurrent refresh calls into a single ajax request', async () => {
		// Arrange.
		const fetchMock = jest.fn().mockResolvedValue( { ok: true, text: () => Promise.resolve( 'fresh-nonce' ) } );
		global.fetch = fetchMock as unknown as typeof fetch;

		// Act.
		const [ firstNonce, secondNonce ] = await Promise.all( [ refreshAuditsNonce(), refreshAuditsNonce() ] );

		// Assert.
		expect( firstNonce ).toBe( 'fresh-nonce' );
		expect( secondNonce ).toBe( 'fresh-nonce' );
		expect( fetchMock ).toHaveBeenCalledTimes( 1 );
	} );

	it( 'issues a new ajax request for a refresh call after the previous one has settled', async () => {
		// Arrange.
		const fetchMock = jest.fn().mockResolvedValue( { ok: true, text: () => Promise.resolve( 'fresh-nonce' ) } );
		global.fetch = fetchMock as unknown as typeof fetch;
