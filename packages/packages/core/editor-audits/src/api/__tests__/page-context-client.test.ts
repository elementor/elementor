...
const NONCE_INVALID_ERROR = {
  response: { status: 403, data: { code: 'rest_cookie_invalid_nonce' } },
};
...
describe( 'fetchPageContext', () => {
  ...
  it( 'refreshes the nonce and retries once when the request fails with an invalid nonce', async () => {
    ...
    const get = jest
      .fn()
      .mockRejectedValueOnce( NONCE_INVALID_ERROR )
      .mockResolvedValueOnce( { data: expected } );
    jest
      .mocked( httpService )
      .mockReturnValue( { get } as unknown as ReturnType< typeof httpService > );
