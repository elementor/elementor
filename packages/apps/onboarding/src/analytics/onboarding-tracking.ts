...
function trackEvent(
  isActive: boolean,
  eventName: string,
  payload: Partial< OnboardingEventPayload >
): void {
...
export function trackLoginType(
  isActive: boolean,
...
export function trackThemeSelected(
  isActive: boolean,
  theme: string,
  source: ThemeSelectedSource
): void {
...
export function trackProFeaturesSelected(
  ...
  trackEvent( isActive, OnboardingEventName.PRO_FEATURES_SELECTED, {
    ...
    interaction_result:
      params.targetName === 'continue_with_free' ? 'finish_onboarding' : 'pricing_page',
...
export function trackSummary( isActive: boolean, snapshot: ObSummarySnapshot ): void {
  ...
  const metadata: ObSummaryMetadataItem[] = [
    ...
    },
    {
      ...
      value: toSummaryValue(
        snapshot.choices.experience_level !== null &&
          snapshot.choices.experience_level !== undefined
  ...
  trackEvent( isActive, OnboardingEventName.SUMMARY, {
    ...
    interaction_description:
      'trigger event upon onboarding completion or when flow is closed abruptly',
