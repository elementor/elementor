...
export function useOnboardingEvent() {
  ...
  return useMemo(
    () => ( {
      ...
      trackConnect: ( success: boolean, error?: string ) =>
        trackConnect( isActive, success, error ),
      ...
      trackUpgradeClicked: ( currentStepId: string ) =>
        trackUpgradeClicked( isActive, currentStepId ),
      trackResumeOnboarding: ( resumeStepId: string ) =>
        trackResumeOnboarding( isActive, resumeStepId ),
