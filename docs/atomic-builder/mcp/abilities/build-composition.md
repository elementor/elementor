...
Creates v4 element trees from XML + parallel maps (`element_config`, `style`, `classes`). Permission: `edit_posts` (+ `edit_post` on `post_id`).

## Public API

| Symbol | Signature | Purpose |
|--------|-----------|---------|
| `Build_Composition_Ability` | `execute( array $input ): array` | Parse XML, apply configs/styles/classes, persist |
| Ability ID | `elementor/build-composition` | Stable MCP host identifier |

Verified: `build-composition-ability.php`.
...
CSS is converted to native atomic styles; unsupported shapes fall back to `custom_css` and `animation` is dropped. See the live tool prompt for the full conversion guide.
