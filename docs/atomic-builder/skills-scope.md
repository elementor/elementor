...
| [extend-prop-types](../../.cursor/skills/extend-prop-types/SKILL.md) | External | **PHP:** third-party plugin repo; plugin-owned prop types and transformers. **Editor TS:** plugin-owned package/bundle registered via `elementor/editor/v2/packages`. **Do not modify Elementor Core.** | Prop type first; transformers when render/import/export differs. Style prop types should also be covered in [internal-extend-css-converter](../../.cursor/skills/internal-extend-css-converter/SKILL.md) (Internal). Changing the *global* prop vocabulary (core `prop-types/`) is Core-only. |
