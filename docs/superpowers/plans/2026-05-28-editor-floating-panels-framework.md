...
> **Update (2026-05-31): preview-mode hide with fade.** Open floating panels fade out when the editor enters preview mode (`#elementor-mode-switcher-preview-input` / `useEditMode() === 'preview'`) and fade back in on return to edit mode. The panel stays mounted so state is preserved. Only applies while a panel is open (e.g. when the Audit Page app-bar toggle is active).
