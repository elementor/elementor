...
module.exports = elementorModules.ViewModule.extend( {
	...
	createLightbox( element ) {
		...
		if ( ! element.dataset.elementorLightboxSlideshow ) {
			...
			this.showModal( {
				...
				id: slideshowID,
				url: element.href,
				hash: element.getAttribute( 'data-elementor-action-hash' ),
	...
	getShareLinks() {
		...
		$.each( socialNetworks, ( key, data ) => {
			const networkLabel = data.label,
				$link = $( '<a>', { href: this.createShareLink( key, itemUrl, $activeSlide.attr( 'data-elementor-action-hash' ) ), target: '_blank' } ).text( networkLabel ),
	...
	setSlideshowContent( options ) {
		...
		options.slides.forEach( ( slide ) => {
			...
			if ( slide.hash ) {
				$slide.attr( 'data-elementor-action-hash', slide.hash );
			}
	...
	openSlideshow( slideshowID, initialSlideURL ) {
		...
		$allSlideshowLinks.each( function() {
			...
			const slideData = {
				...
				hash: this.getAttribute( 'data-elementor-action-hash' ),
			};
