...
class Eicons {
	constructor( env ) {
		...
		this.svgIconsJsonPath = path.resolve( __dirname, '../assets/lib/eicons/eicons.json' );
		this.svgIconsJSDestination = path.resolve( __dirname, '../assets/dev/js/frontend/utils/icons/eicons.js' );
	}

	createFrontendIconsFile() {
		if ( fs.existsSync( this.svgIconsJsonPath ) ) {
			const svgIconsData = JSON.parse( fs.readFileSync( this.svgIconsJsonPath ) );
			...
			write.sync( this.svgIconsJSDestination, svgIconsJSContent );
		}
	}
