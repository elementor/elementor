...
echo "${SEPARATOR}Merging master -> develop"
git checkout develop
git merge origin/master
git push origin develop

# Merge develop -> next release
echo "${SEPARATOR}Merging develop -> next release ${NEXT_RELEASE_BRANCH}"
git checkout "${NEXT_RELEASE_BRANCH}"
git merge origin/develop
git push origin "${NEXT_RELEASE_BRANCH}"

# Merge next release -> next next release
echo "${SEPARATOR}Merging next release ${NEXT_RELEASE_BRANCH} -> next next release ${NEXT_NEXT_RELEASE_BRANCH}"
set +e
bash -e <<TRY
...
echo "${SEPARATOR}Merging next release ${NEXT_RELEASE_BRANCH} -> developer-edition"
