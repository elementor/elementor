...
echo "Merging develop -> next release ${NEXT_RELEASE_BRANCH}"
git checkout "${NEXT_RELEASE_BRANCH}"
git merge origin/develop
git push origin "${NEXT_RELEASE_BRANCH}"

# Merge next release -> next next release
echo "Merging next release ${NEXT_RELEASE_BRANCH} -> next next release ${NEXT_NEXT_RELEASE_BRANCH}"
set +e
bash -e <<TRY
...
echo "Merging next release ${NEXT_RELEASE_BRANCH} -> developer-edition"
