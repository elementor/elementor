				...
				{ 0 === taxonomies.length && <Text>{ __( 'No Results Found', 'elementor' ) }</Text> }
				{
					taxonomies.map( ( taxonomy ) => (
						...
						<label key={ taxonomy.text } className="e-kit-library__tags-filter-list-item">
							<Checkbox
								checked={ !! props.selected[ taxonomy.type ]?.includes( Taxonomy.queryParamSetterTransformer( taxonomy.text ) ) }
								onChange={ ( e ) => {
									const checked = e.target.checked;
									eventTracking( 'kit-library/filter', taxonomy.type, checked, taxonomy.text );
									props.onSelect( taxonomy.type, ( prev ) => {
										return checked
											? [ ...prev, Taxonomy.queryParamSetterTransformer( taxonomy.text ) ]
											: prev.filter( ( tagId ) => tagId !== Taxonomy.queryParamSetterTransformer( taxonomy.text ) );
									} );
								} } />
							{ taxonomy.text }
						</label>
