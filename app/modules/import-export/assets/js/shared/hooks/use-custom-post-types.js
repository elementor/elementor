...
export function useCustomPostTypes( { include = [] } = {} ) {
	...
	useEffect( () => {
		const cpt = Object.assign( {}, elementorAppConfig[ 'import-export' ]?.summaryTitles?.content?.customPostTypes || {} );

		if ( include.length ) {
			Object.entries( elementorAppConfig[ 'import-export' ]?.summaryTitles?.content || {} ).forEach( ( [ postType, post ] ) => {
