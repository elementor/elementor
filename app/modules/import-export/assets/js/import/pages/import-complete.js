...
export default function ImportComplete() {
	...
	const getTemplatesSummary = useCallback( () => {
		...
		const summaryTitles = elementorAppConfig[ 'import-export' ]?.summaryTitles?.templates || {};
	...
	const getContentSummary = useCallback( () => {
		...
		const contentCounts = {};
		const summaryTitles = elementorAppConfig[ 'import-export' ]?.summaryTitles?.content || {};
