...
export default function ImportContextProvider( props ) {
	...
	const isOldExport = isVersionLessThan( data.uploadedData?.manifest?.version, elementorAppConfig[ 'import-export' ].manifestVersion );
	const isOldElementorVersion = isVersionLessThan( elementorAppConfig[ 'import-export' ].elementorVersion, data.uploadedData?.manifest?.elementor_version );
