					...
					items.map( ( item ) => {
						...
						return 'link' === item.linkType
							? <Button
									...
									target="_blank"
									rel="noreferrer"
									className={ `e-kit-library__tags-filter-list-item ${ item.isActive ? 'active-item' : '' } eps-menu-item__link eps-menu-item__external-link` }
									text={ item.text }
									icon={ 'eicon-editor-external-link' }
							/> : <Button
								key={ item.text }
								text={ item.text }
								className={ `e-kit-library__tags-filter-list-item ${ item.isActive ? 'active-item' : '' } eps-menu-item__link` }
								url={ item.url }
							/>
					} )
				}
