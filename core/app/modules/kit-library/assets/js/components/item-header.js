...
function useKitCallToActionButton( model, { apply, isApplyLoading, onConnect } ) {
	...
	return useMemo( () => {
		...
		if ( type === TYPE_PROMOTION && subscriptionPlan ) {
			const buildSubscriptionLink = () => {
				let url = subscriptionPlan.promotion_url;
				if ( model.title ) {
					// Remove special characters, replace spaces with  turl kit name to lowercase.
					let cleanTitle = model.title.replace( /[^\w\s]/g, '' ).replace( /\s/g, '-' ).toLowerCase();
