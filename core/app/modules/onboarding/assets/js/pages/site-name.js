	...
	const { state, updateState, getStateObjectToUpdate } = useContext( Context ),
		...
		actionButton = {
			...
			onClick: () => {
				...
				if ( actionButtonRef.current.value !== state.siteName && '' !== actionButtonRef.current.value ) {
					setAjax( {
						data: {
							action: 'elementor_update_site_name',
							data: JSON.stringify( {
								siteName: actionButtonRef.current.value,
							} ),
						},
					} );
				} else if ( actionButtonRef.current.value === state.siteName ) {
	...
	useEffect( () => {
		if ( 'initial' !== ajaxState.status ) {
			if ( 'success' === ajaxState.status && ajaxState.response?.siteNameUpdated ) {
				...
				stateToUpdate.siteName = actionButtonRef.current.value;
