...
export default class CommandBaseOriginal extends ArgsObject {
	...
	constructor( args = {}, commandsAPI = $e.commands ) {
		...
		if ( ! $e.commands.constructor.trace.length ) {
			...
			if ( ! elementorCommonConfig.isTesting && -1 === this.instanceTypes.indexOf( Command.getInstanceType() ) ) {
				throw Error( `'${ CommandBaseOriginal.getInstanceType() }' Class cannot be extended directly, use: '${ Command.getInstanceType() }'.` );
			}
