...
/**
 * Thin wrapper around the mixpanel/mixpanel-php SDK, isolating it behind an Elementor-owned API so
 * the rest of the codebase never depends on the vendor library directly.
 */
class Mixpanel_Server_Client {

	const HOST = 'api-eu.mixpanel.com';

	public static function track( string $event_name, array $properties = [] ): bool {
		// Guard against the SDK's global (non-namespaced) `Mixpanel` class colliding with another
		...
		try {
			// `Mixpanel::getInstance()` is itself a singleton keyed by token - it returns the
			// existing instance or creates a new one if it doesn't exist yet.
			$mixpanel = Mixpanel::getInstance( ELEMENTOR_EDITOR_EVENTS_MIXPANEL_TOKEN, [
				'host' => self::HOST,
				'consumer' => 'wp',
				'consumers' => [
