...
class Manager {
	...
	private function create_default() {
		return $this->create( [ 'post_title' => esc_html__( 'Default Kit', 'elementor' ) ] );
	}
	...
	private function before_delete_kit( $post_id, $is_permanently_delete = false ) {
		...
		wp_die(
			new \WP_Error( 'cant_delete_kit', $confirmation_content ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		);
	}
