			...
			'meta' => [
				...
				'annotations'  => [
					'instructions' => implode( "\n", [
						...
						'LEAF widgets for `widget`: "heading" → e-heading; "paragraph"|"label"|"text" → e-paragraph; "button" → e-button; "image" → e-image. Leaves have text/tag/url/attachment_id as applicable. Use label or text for kicker/caption copy that should not carry heading semantics — these map to e-paragraph and accept any css.',
						'BUTTON defaults: renders as <a> (pass tag:"button" for the semantic button element). Base reset: align-self:flex-start so the button sizes to content inside a flex-column. When no css is provided, Elementor\'s base style applies — blue background (#375EFB), white text, 12px 24px padding, 2px border-radius. Pass css to override. url wires the href.',
						...
						'GRADIENT TEXT: use the text-gradient: shorthand in css — e.g. text-gradient: linear-gradient(135deg, #e2e8f0 0%, #a78bfa 60%, #60a5fa 100%); — instead of writing 4 separate CSS declarations. The ability expands it internally to the 4 required declarations in custom_css and reports it as a single css_gap entry.',
						'BORDER: border: N px solid <color> is parsed and decomposed into typed border-width, border-style, and border-color props — 0 gaps. rgba() colors are fully supported. Only border shorthands with unsupported syntax fall back to custom_css.',
						'CSS GAPS: CSS declarations that cannot be represented as typed v4 props (gradients, background-clip, vendor-prefixed props, etc.) are automatically collected into the style variant\'s custom_css field and rendered as raw CSS — no validation failure. Each gap entry includes a hint field. dry_run:true already returns css_gaps before save so you can inspect and adjust the spec before committing.',
						'PADDING RESET: both e-flexbox and e-div-block containers automatically receive a padding:0 base reset — the 10px Elementor default is suppressed on all containers, inner and outer, without needing explicit padding:0 in css.',
