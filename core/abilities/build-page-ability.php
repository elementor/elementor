			...
			'meta' => [
				...
				'annotations'  => [
					'instructions' => implode( "\n", [
						...
						'FLEX PROP (proportional column widths): set on child e-flexbox elements in their local styles. {"$$type":"flex","value":{"flexGrow":{"$$type":"number","value":2}}} gives that child 2× the space of a sibling with flexGrow:1. flexShrink and flexBasis are optional shape fields. Easiest path: use elementor/make-layout which auto-wires flex-grow from a weight spec.',
						'LAYOUT SHORTCUT: elementor/make-layout { direction:"row", columns:[{weight:2,elements:[...]},{weight:1,elements:[...]}] } returns a fully-wired outer e-flexbox node with weighted column containers — replaces building the nested flexbox tree by hand and keeps the build-page payload small enough to stay under the MCP object-size limit.',
						'PAYLOAD SIZE: build-page has an MCP object-size limit (~20–25KB serialised JSON). For large trees, compose with make-layout + make-widget to generate compact nodes. If a page is still too large for one call, save the skeleton with build-page then add sections with append-element.',
						'WIDGET BASE STYLES (props already set by the widget stylesheet — override in local styles only when you need different values):',
						'  e-heading:   margin:0px (browser default heading margin removed). Link wrapper (when used): all:unset; cursor:pointer.',
						'  e-paragraph: margin:0px (browser default <p> margin removed). Link wrapper: all:unset; cursor:pointer.',
						'  e-button:    background:#375EFB; display:inline-block; padding:12px 24px; border-radius:2px; border-width:0; text-align:center; color:inherit; text-decoration:none. color:inherit inherits from the nearest ancestor with an explicit colour — always set an explicit color in the button local style for ghost/outline variants.',
						'  e-image:     display:block (removes inline baseline gap). Link wrapper (when used): display:inherit; width:fit-content.',
						'  e-flexbox:   display:flex; flex-direction:row; padding:10px. The 10px padding is a base default — override to 0 when you want a flush container.',
						'  div-block:   display:block; padding:10px; min-width:30px.',
						'  e-divider:   border:none; background:#000000; height:1px. Override background to change divider colour.',
						'  e-svg:       display:inline-block; width:65px; height:65px. Override width/height for custom icon sizes.',
						'  e-youtube:   aspect-ratio:16/9; overflow:hidden. Maintains 16:9 ratio automatically.',
						'  e-video (self-hosted): max-width:100vw; display:inline-block; aspect-ratio:16/9.',
