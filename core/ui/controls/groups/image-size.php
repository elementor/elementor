<?php
namespace Elementor\Core\UI\Controls\Groups;
	...
	public static function get_attachment_image_html( $settings, $image_size_key = 'image', $image_key = null ) {
		...
		if ( ! empty( $image['id'] ) && in_array( $size, $image_sizes ) ) {
			...
			if ( ! empty( $image_src ) ) {
				...
				$html .= sprintf( '<img src="%s" title="%s" alt="%s"%s />', esc_attr( $image_src ), Media_Control::get_image_title( $image ), Media_Control::get_image_alt( $image ), $image_class_html );
			}
		}
